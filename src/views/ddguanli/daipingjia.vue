					...
					<div class="content">
						...
						<div class="tablebox">
							...
							<el-table :data="tableData1" style="width: 100%" row-key="id" border stripe lazy :tree-props="{children: 'children', hasChildren: 'hasChildren'}">
								<el-table-column label="订单号" width="220" @row-click="lookinfo" v-if="rfstate != 0">
					...
					<table class="table" align="center" border="1" cellpadding="0px" cellspacing="0px">
						<tr>
							...
							<td colspan="3" style="color: red;">
								...
								<span v-if="dantian.state==3">待评价</span>
		...
		methods: {
			...
			oderxq() { //分页查询
				...
				oderxq('order/getTShopTypePageList', params).then(res => {
					...
					var arr = res.resultData.content;
					// var newarr = []
					// for (let i in arr) {
					// 	if (!arr[i].rfstate) {
					// 		newarr.push(arr[i])
