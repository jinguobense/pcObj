			...
			<el-container>
				...
				<el-container>
					<el-main>
						...
						<div class="content">
							<div class="titlebar">
								<span class="fenl1">首页轮播广告列表</span>
								<div class="yoou">
									<div @click="xinjian">
							...
							<div class="searchbox">
								<div class="search">
									<el-select size="medium" v-model="urltype" placeholder="链接类型"  @change="sousu">
										<el-option key="0" value="0" label="富文本"></el-option>
										<el-option key="1" value="1" label="店铺"></el-option>
										<el-option key="2" value="2" label="商品"></el-option>
										<el-option key="5" value="5" label="满减活动列表"></el-option>
										<el-option key="6" value="6" label="满送活动列表"></el-option>
										<el-option key="7" value="7" label="限时抢购列表"></el-option>
										<el-option key="8" value="8" label="大众拼团列表"></el-option>
										<el-option key="9" value="9" label="2人拼列表"></el-option>
									</el-select>
								 ...
								 @selection-change="handleSelectionChange" @select="deld" @select-all="quanxuan" stripe >
									...
									<el-table-column prop="urltype" label="连接类型">
										<div slot-scope="scope">
											...
											<span v-if="scope.row.urltype==1">店铺</span>
											<span v-if="scope.row.urltype==2">商品</span>
											<span v-if="scope.row.urltype==5">满减活动</span>
											<span v-if="scope.row.urltype==6">满送活动</span>
											<span v-if="scope.row.urltype==7">限时抢购</span>
											<span v-if="scope.row.urltype==8">大众拼团</span>
											<span v-if="scope.row.urltype==9">2人拼</span>
										</div>
									</el-table-column>
								...
								<div class="tablebox">
										...
										<tr>
											...
											<td class="ggtu1">
												<div style="margin-left: 20px;">
													<el-input v-model="xuhaol" placeholder="输入序号" oninput="value=value.replace(/[^\d]/g,'')"></el-input>
												</div>
											</td>
										</tr>
									</table>
									<table align="center" border="1" cellpadding="0px" cellspacing="0px" class="biaoge">
										<tr>
											...
											<td class="changxuanze">
												<div class="xuanzechang2">
													<el-select size="medium" v-model="ljlx" placeholder="链接类型" @focus="xigai1=3">
														<el-option key="9" value="9" label="2人拼"></el-option>
														<el-option key="8" value="8" label="大众拼团"></el-option>
														<el-option key="7" value="7" label="限时抢购"></el-option>
														<el-option key="6" value="6" label="满送活动"></el-option>
														<el-option key="5" value="5" label="满减活动"></el-option>
														<el-option key="0" value="0" label="富文本"></el-option>
														<el-option key="2" value="2" label="商品"></el-option>
														<el-option key="1" value="1" label="店铺"></el-option>
													</el-select>
										...
										<tr v-if="ljlx==2 || xigai1==0">
											...
											<td class="changxuanze1" colspan="3">
												<div class="xuanzechang2">
													<el-input v-model="goodnamel" placeholder="选择" @focus="dakai1"></el-input>
												</div>
											</td>
										</tr>
										<tr v-if="ljlx==0 || xigai1==2">
											<td class="fuwenben1">
												<span>
													富文本框:
												</span>
											</td>
											<td class="fuwenben" colspan="3" >
												<div style="height: 400px;" >
													<quill-editor v-model="danswer" ref="myQuillEditor"  style="height: 300px; margin:10px;"  
						...
						<div class="addbox" v-if="chanjiaxz">
							...
							<div class="addwrap">
								...
								<div class="hezi">
									<div>
										<el-input v-model="ssssname" placeholder="店铺名称"></el-input>
								...
								<div class="biaoge1">
									<el-table ref="multipleTable" :data="shoplist" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange"
									 @select="danxuan" stripe border>
										...
										<el-table-column label="分类名称" prop="shoptypename" width="120">
											<!-- <template slot-scope="scope">
												<el-image style="width: 54px; height: 56px" :src="scope.row.typeName">
												</el-image>
											</template> -->
										</el-table-column>
						...
						<div class="addbox" v-if="shopxz">
							...
							<div class="addwrap">
								...
								<div class="hezi">
									<div>
										<el-input v-model="changjianame" placeholder="店铺名称"></el-input>
								...
								<div class="biaoge1">
									<el-table ref="multipleTable" :data="goodlist" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange"
									 @select="danxuan1" stripe border>
										...
										<el-table-column prop="shopName" label="店铺名称" width="263">
										</el-table-column>
...
<script>
	...
	// 使用 <quill-editor v-model="nanswer" ref="myQuillEditor" style="height: 400px;" :options="editorOption">
	// 								</quill-editor>
	import {
		Gglunbolist,
		Ggshoplist,
	...
	export default {
		...
		methods: {
			...
			Gggoodsadd() {
				...
				if (this.ljlx == "商品") {
					this.ljlx = 2
				} else if (this.ljlx == "富文本") {
					this.ljlx = 0
				} else if (this.ljlx == "店铺") {
					this.ljlx = 1
				} else if (this.ljlx == "满减活动列表") {
					this.ljlx = 5
				} else if (this.ljlx == "满送活动列表") {
					this.ljlx = 6
				} else if (this.ljlx == "限时抢购列表") {
					this.ljlx = 7
				} else if (this.ljlx == "大众拼团列表") {
					this.ljlx = 8
				} else if (this.ljlx == "2人团列表") {
					this.ljlx = 9
				}
				if (this.spzt == "显示") {
					this.spzt = 0
			...
			open() {
				if(this.goodsidlist==""){
					...
					this.$confirm('确认要删除该店铺分类记录吗?', '系统提示', {
