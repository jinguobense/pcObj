			...
			<el-container>
				...
				<el-container>
					<el-main>
						...
						<div class="content">
							<div class="titlebar">
								<span class="fenl1">商品一级分类广告列表</span>
								<div class="yoou">
									<div @click="xinjian">
							...
							<div class="searchbox">
								<div class="search">
									<el-select size="medium" v-model="urltype" placeholder="链接类型" @change="sousu">
										...
										<el-option key="3" value="3" label="平台商品二级分类"></el-option>
							...
							<div class="tablebox">
								<el-table ref="multipleTable" :data="tableData" border tooltip-effect="dark" style="width: 100%"
								 @selection-change="handleSelectionChange" @select="deld" @select-all="quanxuan" stripe>
									...
									<el-table-column prop="pfgtypename2" label="平台商品二级分类名称">
									</el-table-column>
							...
							<div class="addwrap">
								...
								<table align="center" border="1" cellpadding="0px" cellspacing="0px" class="biaoge">
									<tr>
										...
										<td class="ggtu">
											<span>平台商品一级分类名称：</span>
										</td>
										<td class="ggtu1">
											<div style="margin-left: 20px;">
												<el-select size="medium" v-model="yijifeilei" placeholder="请选择" @focus="huoqu">
													<div v-for="(item,index) in yilifeil" :key=index>
								...
								<table align="center" border="1" cellpadding="0px" cellspacing="0px" class="biaoge">
									...
									<tr v-if="ljlx==3 || xigai1==1">
										...
										<td class="changxuanze">
											<div class="xuanzechang2">
												<el-select size="medium" v-model="erjifeilei" placeholder="链接类型" @focus="erji">
													<div v-for="(item,index) in yilifeil" :key=index>
														<el-option :key="item.id" :value="item.id" :label="item.name"></el-option>
													</div>
												</el-select>
						...
						<div class="addbox" v-if="shopxz">
							...
							<div class="addwrap">
								...
								<div class="searchbox">
									<div class="search">
										<el-input v-model="changjianame" placeholder="厂家名称" @change='goodsouso'></el-input>
										<el-input v-model="shopname6" placeholder="商品名称" @change='goodsouso'></el-input>
									</div>
									<div class="btngrap">
										<img src="../../assets/img/sousu.png" alt="" @click="goodsouso">
										<div class="shuax" @click="goodchongzhi">
											<img src="../../assets/img/shuaxinb.png" alt="">
											<span class="">重置</span>
										</div>
									</div>
								...
								<div class="tablebox">
									<el-table ref="multipleTable" :data="goodlist" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange"
									 @select="danxuan1" stripe border>
										...
										<el-table-column prop="name" label="商品名称" width="" show-overflow-tooltip="name">
										</el-table-column>
	...
	export default {
		...
		methods: {
			...
			Gggoodsfeilei() {
				...
				Gggoodsfeilei('platform/getChildren', params).then(res => {
					this.yilifeil = res.resultData
					console.log(this.yilifeil)
				})
			},
			erji() {
				// console.log(this.dantiao.pfgtypeid1)
				this.parentId = this.dantiao.pfgtypeid1
				this.parentId = this.yijifeilei
				this.Gggoodsfeilei()
			},
			huoqu() {
				this.parentId = 0
				this.erjifeilei = ''
				console.log("44545456")
				this.Gggoodsfeilei()
			},
			quxiao() {
			...
			tijiao() {
				if(this.imageUrl==''){
					...
					return
				}else if(this.yijifeilei==''){
					this.$message({
						type: 'error',
						message: '平台商品一级分类不能为空'
					});
					return
				}
				this.dialogVisible = false
			...
			Gggoodsadd() {
				...
				if (this.yijifeilei == this.dantiao.pfgtypename1) {
					this.yijifeilei = this.dantiao.pfgtypeid1
				}
				if (this.erjifeilei == this.dantiao.pfgtypename2) {
					this.erjifeilei = this.dantiao.url
				}
				let params = {
					type: 1,
					urltype: this.ljlx,
			...
			deleteRow(e) {
				...
				this.imageUrl = e.img
				this.yijifeilei = e.pfgtypename1
				this.goodnamel = e.goodsname
				// this.id=e.url
				this.erjifeilei = e.pfgtypename2
				if (e.urltype == 2) {
					this.xigai1 = 0
				} else {
					this.xigai1 = 1
